...
class SC2Division {
  ...
  public function parseDivision()
  {
	...
	private function getBNETDivisionInfo()
	{
		...
		// Get division name
		$nameNode = $divisionHTML->find('.data-title .data-label span', 0);
		if ( $nameNode ) {
		...
		// Get division bracket
		$divider = $divisionHTML->find('.data-title .data-label', 0)->plaintext;
		$bracket = GeneralUtils::parseInt($divider);
		...
		foreach ( $rankingNodes as $rankingNode ) {
			...
			$playerDivision = array();
			{
			  // Get joined date
	  			...
	  			// $playerDivision['joinedDate'] = date('Y-m-d', $playerDivision['joinedDate']);
